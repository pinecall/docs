...
([writing-an-agent.md](/agent/writing-an-agent)).
...
A project of several with nobody named is refused with the names: `this project has 2 agents: name one with --agent dispatch or --agent sales`. A directory with no `agents/` at all is one somebody
...
world attached, for these words ([writing-an-agent.md](/agent/writing-an-agent)). Which base is the
