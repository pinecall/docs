---
title: "Settings"
description: "What the org set for an agent — vendors, models, the opening, what it knows by heart, its bases — per world, per corner, a version a row; and the org's words."
"og:see_also": "https://github.com/pinecall/runtime/docs/protocol/settings-api.md"
---
...
the bases **by name**, with the scope it lacks: `llm: the pipeline's, and this key does not open pipeline: it opens …`. Their set carries those fields over untouched from what stands.
...
`k`, `min_score`); a turn's search fans out over all of them ([gateway-api.md](/api/gateway),
...
when it opened (`config_version`, `lexicon_version`, 0037): `{config_version, lexicon_version, config: TuningRow \| null, lexicon: LexiconRow \| null}`. A reviewer of Tuesday's call sees Tuesday's
...
of `LexiconRow`, each `{holder, version, author, note, set_at, lexicon: {said: [{word, spoken}], heard: [string]}}`. `PUT {lexicon, if_version, note, team}` is the whole lexicon, with the same
