---
title: "The wire"
description: "One schema, generated into Python, TypeScript and Ruby, so the three cannot disagree."
"og:see_also": "https://github.com/pinecall/protocol/docs/README.md"
---
...
| page | holds |
|---|---|
| [events.md](/wire/events) | every event, one line each, with the page its data shape is on |
| [events-call.md](/wire/events-call) | `call.*`, `user.*`, `agent.state`, `agent.transcript`, `turn.*`, `metrics.*`: the call and the conversation, field by field |
| [events-app.md](/wire/events-app) | `tool.*`, `state.changed`, `prompt.changed`, `tools.changed`, `confirm.*`, `memory.ops`, `docs.sources`, `custom` |
| [events-control.md](/wire/events-control) | `supervisor.*`, `log.gap`, `log.caught_up`, `error`, `pong`, `agent.registered`, `agent.configured` |
| [events-room.md](/wire/events-room) | `room.*`, `participant.*`, `track.*`, `event.received`: the room's facts and the outside world's |
| [commands.md](/wire/commands) | every command with what it lands as, and the supervise verbs |
| [metrics.md](/wire/metrics) | the metrics table: block · field · unit · required · measured by · meaning |
| [state.md](/wire/state) | what a log reduces to |
| [shapes.md](/wire/shapes) | the envelope and the shared shapes |
| [projections.md](/wire/projections) | written by hand: what `public` and `tenant` keep, and the DataChannel topics |
| [operator-api.md](/api/operator) | written by hand: `/v1/ops/*`, the ops key, and the routes doors |
| [tokens.md](/api/tokens) | written by hand: `POST /v1/tokens`, LiveKit's token endpoint with the org check, the contact id and single use in front of it |
