...
`render()` method is refused when the class is defined: `ClinicaNorte declares both @render(ClinicaPrompt) and a render() method; two ways to answer one question — keep one`.
