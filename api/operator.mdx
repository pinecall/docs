---
title: "Operator API"
description: "The box's own doors: orgs, people, keys, plans and the fleet."
"og:see_also": "https://github.com/pinecall/runtime/docs/protocol/operator-api.md"
---
...
[../multi-tenancy.md](/operate/multi-tenancy), and the verbs are [../the-runtime-cli.md](/operate/the-runtime-cli). What the operator configures about the **box itself** — its mail, its brand, a box-wide "Continue with Google" — is [the-box.md](/api/the-box).
...
shown beside what was spent (`GET /v1/insights`, [console-api.md](/api/console)); nothing is refused over it. Four of them are **flows** — what the org has consumed, or holds open right now. `minutes` is
...
- **`PUT /v1/knowledge/{base}`** counts what the push would become — the org's other bases plus the
  ...
  row is written, since a push is all or nothing: `org clinica-norte has used 2400 of its 2000 knowledge_chunks: credits.exhausted`. It is the one refusal here that writes **no**
...
**`numbers`** caps what the box may BUY for the org on its own carrier account (`POST /v1/numbers/buy`, [numbers](/api/numbers)), counted on its bought routes (`managed`); one the tenant
...
An API key is what a worker and a tenant's app knock at the runtime's own doors with — `GET /v1/routes`, the app socket, the log. It **is** the org: every door reads the org off the key, none
...
nothing; the box's two doors over it — `GET /v1/ops/orgs/{org}/sso`, and the break-glass `PUT …/sso/required {required}` that lets a password open that org again — are in [people.md](/api/people).
...
in memory, and these doors read it. [../scaling.md](/run/scaling) is the whole picture.

### `GET /v1/ops/fleet`

`{now, stale_after_s, workers: [{worker, active, max_jobs, load, draining, cordoned, seen_at}], totals: {workers, active, seats, free, accepting, full, busy}}` — every worker ever heard from,
...
with `PINECALL_OPS_KEY`; every verb, flag by flag, is [../the-runtime-cli.md](/operate/the-runtime-cli).
...
org's API key, and have a page of their own: [pipeline-api.md](/api/pipeline).
