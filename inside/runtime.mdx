...
description: "Eighteen packages, the import table, and the path a call takes through them."
"og:see_also": "https://github.com/pinecall/runtime/ARCHITECTURE.md"
---
...
Twenty-five tables, thirty-five migrations (`migrations/00NN_*.sql`, applied in order by `migrate up`, the one `.post.sql` by `migrate up --post`; `0008_memory` holds the contact's facts and
...
why — per tool, so a `recall` that answered is used beside a `search` that did not; with `docs.mode = "tool"` the platform runs nothing and the model calls `search` itself, through the same callable.
...
`lookups/`, in-process — one `Lookups(memory, knowledge, logs, calls, keys_of, quotas_of, may_remember)` per process, implementing both protocols, that recalls the contact's facts (the
