...
description: "The dev stack, the images this repo builds, and what runs where."
"og:see_also": "https://github.com/pinecall/runtime/infra/README.md"
---
...
One tool belongs on the machine and not in the compose file: **the LiveKit CLI**, `brew install livekit-cli` (Linux: `curl -sSL https://get.livekit.io/cli \| bash`). It reads
...
- **A stale checkout needs `uv sync` after the lockfile moves.** `cannot import name deepgram from livekit.plugins` and a wall of type errors mean the virtualenv is older than `uv.lock`:
  `scripts/bootstrap`.
