...
- **What only the agent's directory can answer** — the class for a written call, the personas, the
  goldens, the docs folder — the page asks the gateway, and the gateway asks the `pinecall start` standing there (`lib/dev.ts`; the runtime's `docs/protocol/dev-verbs.md`).
