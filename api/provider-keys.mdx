---
title: "Provider keys"
description: "An org bringing its own vendor keys, and where they are kept."
"og:see_also": "https://github.com/pinecall/runtime/docs/protocol/provider-keys.md"
---
Section 6 of [gateway-api.md](/api/gateway), on a page of its own: an org's own vendor keys,
...
**One door does read them back** — `GET /v1/agents/{slug}/provider-keys` answers `{"keys": {vendor: key}}` in the clear — and it is the worker's: an org's own keys, to the org's own process, on the
