...
issues a key. Who a key belongs to is [multi-tenancy.md](/operate/multi-tenancy).
...
The whole path, with every output under it, is [from-zero.md](/start/from-zero).
...
[protocol/gateway-api.md](/api/gateway). It needs, at the least, one key of each
...
has a mailbox ([the-box.md](/api/the-box)); the verb prints the link either way and does
...
has one prints no link and seats them `active` (`already a person on this box: seated, they sign in with the password they have`). `operator` makes a member, by email, an operator of this box — their
...
— wiring one is the org's own door ([protocol/people.md](/api/people)).
...
`loop` is the fleet loop ([scaling.md](/run/scaling)): every `--every` seconds it reads the roster
...
script under `infra/fleet/` or a path to yours ([../infra/fleet/README.md](https://github.com/pinecall/runtime/blob/main/docs/../infra/fleet/README.md));
...
[from-zero.md](/start/from-zero) is this same path with every output under it, through to a call.
