...
(`pinecall memory eval`, the golden `test/<name>/goldens/memory.json`), the index (`pinecall docs eval`, `test/<name>/goldens/docs.json`) and why a call has no retrieval score are in
...
secrets as `PINECALL_KEY`. A red run stops the deploy. [production.md](/operate/production) is the rest.
