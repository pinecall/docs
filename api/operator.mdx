...
[../multi-tenancy.md](/operate/multi-tenancy), and the verbs are [../the-runtime-cli.md](/operate/the-runtime-cli).
...
is the org's people as the operator reads them — `{members: [...], seated}`. `POST /v1/ops/orgs/{named}/members {email, name, role, agents?}` **invites** one — the org's first admin
...
tenant's team. Changing and disabling are the tenant's own `/v1/members` ([people.md](/api/people)).
