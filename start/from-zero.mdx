---
title: "From zero"
description: "Three clones, a database and an agent answering — every command run in order, with the output it returned."
"og:see_also": "https://github.com/pinecall/runtime/docs/from-zero.md"
---
...
[the-runtime-cli.md](/operate/the-runtime-cli) and the agents repo's `docs/the-cli.md` are the reference
...
[a-box-in-production.md](/run/a-box-in-production).
...
sandbox, and the sandbox is watched on your own machine: `pinecall serve` — or `pinecall start --serve`, both in one terminal — puts the console on `http://localhost:4100`, forwarding every
...
token** is the other kind, made in the console — Tokens ▸ New server token, `POST /v1/keys {label, env}` — by a person whose key opens `app` (production's only with production access). It holds
...
an admin — with who made each and when it was last used. Revoke on a row (`POST /v1/keys/{fingerprint}/revoke`) stops your own, one you made, or any for a key that opens `keys`. Revoking keeps the row, so the
...
the row prints `active`, and under it `already a person on this box: seated, they sign in with the password they have`. A login that names no org lands in the oldest of theirs; the console's org
switch moves between them.

**A role is a preset of scopes and nothing more.** A developer opens `app · calls · evals · knowledge · memory · pipeline · supervise · talk`; `numbers`, `keys`, `providers`, `team` and
