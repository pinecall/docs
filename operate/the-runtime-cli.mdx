---
title: "pinecall-runtime"
description: "Every verb of the CLI an operator runs: the gateway, the worker, the orgs, the keys."
"og:see_also": "https://github.com/pinecall/runtime/docs/the-runtime-cli.md"
---
The operator's terminal: the two processes, the database, the tenants and the box. One module per
group, one parser each; `pinecall-runtime` with nothing after it prints them all, and `<group> --help` that group's verbs. The **tenant's** terminal is `pinecall` — the agents repo's
`docs/the-cli.md` — and they never overlap: nothing here writes an agent, nothing there issues a
key. Who a key belongs to is [multi-tenancy.md](/operate/multi-tenancy).
...
[protocol/gateway-api.md](/api/gateway). It needs, at the least, one key of each provider
...
never a password, and the invitation takes none of the org's seats). The door it knocks at, `POST /v1/ops/orgs/{org}/members`, also **mails** that link to the person when the box or the org has a
mailbox ([the-box.md](/api/the-box)); the verb prints the link either way and does not say
...
that already has one prints no link and seats them `active` (`already a person on this box: seated, they sign in with the password they have`). `operator` makes a member, by email, an operator of this
...
one is the org's own door ([protocol/people.md](/api/people)). `quota` replaces **the whole
...
the fleet loop ([scaling.md](/run/scaling)): every `--every` seconds it reads the roster and the
...
`infra/fleet/` or a path to yours ([../infra/fleet/README.md](https://github.com/pinecall/runtime/blob/main/docs/../infra/fleet/README.md)); the
...
[from-zero.md](/start/from-zero) is this same path with every output under it, through to a call. **This
...
[multi-tenancy.md](/operate/multi-tenancy) says what a key IS, and what the tenant does with the one that
