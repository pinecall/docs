...
**One more header**: `pinecall-corner: <member id>` answers an HTTP door in that member's sandbox corner — an admin opening a colleague's copy ([multi-tenancy.md](/operate/multi-tenancy)).
...
worker alone (`providers`): [provider-keys.md](/api/provider-keys).
...
it (`numbers`). `GET /v1/keys` is the org's own API keys by fingerprint, never a key; `POST /v1/keys {label?, env?, scopes?}` mints one for a machine — `app` and production when nothing is
