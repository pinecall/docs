...
description: "What a lookup returns never becomes part of the prompt. The contract, and why."
"og:see_also": "https://github.com/pinecall/runtime/docs/security/prompt-injection.md"
---
...
the caller had said so far — `¿Cuánto cuesta una revisión` for a turn that ended `¿Cuánto cuesta una revisión dental?` — and **the `tool_use` carries the words that were actually sent**, never the
