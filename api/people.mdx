...
dead in a week — and the person accepts at `POST /v1/invitations/{token}` with `{password, env?, device?}` (no key at that door; twelve characters at least, argon2id at rest) — which is what the
...
whatever the password. Or it takes `{code, device?}`: a key holder minted the code at `POST /v1/login/codes` (five minutes, one use), which is how `pinecall run` prints
